...
class UserRoleDepartmentModel extends Model
{
    ...
    public function getSubordinatesWithPpmpStatus(int $currentUserId, int $departmentId): array
    {
        ...
        ->join('users_tbl', 'user_role_department_tbl.user_id = users_tbl.user_id')
        ->join('tasks_tbl as tasks', "tasks.submitted_to = users_tbl.user_id AND tasks.task_type = 'PPMP Assignment' AND tasks.is_deleted = 0", 'left')
    ...
    public function getSubordinatesWithPrStatus(int $currentUserId, int $departmentId): array
    {
        return $this->select([
            'users_tbl.user_id',
            'users_tbl.user_tupid',
        ...
        ->join('users_tbl', 'user_role_department_tbl.user_id = users_tbl.user_id')
        ->join('tasks_tbl as tasks', "tasks.submitted_to = users_tbl.user_id AND tasks.task_type = 'PR Assignment' AND tasks.is_deleted = 0", 'left')
        ->where('user_role_department_tbl.department_id', $departmentId)
        ->where('users_tbl.user_id !=', $currentUserId)
        ->groupBy([
            'users_tbl.user_id',
